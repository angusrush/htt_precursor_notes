...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  i.e.\ if for each $f\colon A \to \Maps(B, C)$ there is a unique map $\tilde{f}\colon A \times B \to C$ making the above diagram commute.
\end{proof}
...
Recall that we imagine simplicial sets as being made of simplices glued together along shared edges. The functor $\alpha$ gives us a model of an $n$-simplex internal to $\mathcal{C}$, and the functor translates
