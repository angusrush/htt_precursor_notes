...
Applying a simplicial set to the $i$th face function gives a map from $k$-simplices to $(k-1)$-simplices.
...
We have said that we should view simplicial sets as built by gluing simplices of all different degrees together along their faces. Face maps specify in which way we should do this gluing. Recall that our simplices have ordered vertices---the vertices of an $n$-simplex run from $0$ to $n$. The $i$th face map takes an $n$-simplex to its $i$th face; that is, the face opposite the vertex labelled by $i$.
...
\begin{example}
  ...
  \end{equation*}
  taking the six 1-simplices of $\Delta^{2}$ (three of which are degenerate) to the three 0-simplices.
  ...
  Similarly, $d_{0}$ takes the edge between $0$ and $1$ to the vertex $1$, and the edge from $0$ and $2$ to the vertex $2$. Put more concisely, and as should now be clear from the form of $\partial_{0}$, we can see that $d_{0}$ takes any 1-simplex from $a$ to $b$ to the zero-simplex $b$. That is, taking the vertices of a 1-simplex as numbered in order, \emph{$d_{0}$ deletes the zeroth vertex.}
  ...
  \begin{equation*}
    ...
    \end{tikzpicture}
  \end{equation*}
  Which element of $\Delta^{2}_{0}$ does the face map $d_{0}$ send $f$ to? Recall that $d_{0}$ comes from the face function $\partial_{0}$, which we can draw like this.
  \begin{equation*}
...
So far, we have talked a lot about simplicial sets, namely functors $\Delta\op \to \Set$. From category theory, one feels that the natural notion of a morphism of simplicial sets should be that of a natural transformation between functors. However, simplicial sets are geometric objects, and we have some idea, on a geometrical level, of what we would like morphisms between them to look like. Fortunately, these notions coincide.
