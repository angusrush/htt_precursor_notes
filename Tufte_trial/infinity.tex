...
\begin{lemma}
  ...
  \begin{itemize}
    \item There exists a category $\mathcal{C}$ and an isomorphism $K \cong N(\mathcal{C})$.
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item Let $\mathcal{C}$ be a small category. Then $\h N(\mathcal{C}) \cong \mathcal{C}$.

    \item For every topological space $X$,
      \begin{equation*}
        \h \Sing(X) \cong \pi_{\geq 1} (X)
      \end{equation*}
...
\begin{definition}[simplicial category]
  ...
  A category enriched over $\SSet$ (with the Cartesian monoidal structure) is called a \defn{simplicial category}.\sidenote{This terminology, while common, is somewhat misleading. Generally, the phrase \emph{simplicial foo} is used to refer to a functor from $\Delta\op$ to the category of foos. However, in this case, \emph{simplicial category} refers to a category enriched over simplicial sets. While this is an abuse of terminology, it is not as extreme as it may appear. After unwinding the definitions, one finds that a simplicial object in $\mathbf{Cat}$ which is \emph{constant on objects} is equivalent to a $\SSet$-enriched category.} Denote the category whose objects are simplicial categories and whose morphisms are enriched functors by $\mathbf{Cat}_{\Delta}$.
\end{definition}
...
\begin{proof}
  ...
  \begin{align*}
    \Sing(X \times Y) &\cong \mathbf{Top}(\abs{\Delta^{\bullet}}, X \times Y) \\
    &\cong \mathbf{Top}(\abs{\Delta^{\bullet}}, X) \times \mathbf{Top}(\abs{\Delta^{\bullet}}, Y) \\
    &\cong \Sing(X) \times \Sing(Y).
  \end{align*}


  The singular set $\Sing(\abs{\Delta^{0}}) \cong \Delta^{0}$, so we don't have any choice in defining $\phi$.
\end{proof}
...
\begin{equation*}
  N(\mathcal{C} \star \mathcal{D}) \cong N(\mathcal{C}) \star N(\mathcal{D}).
\end{equation*}
...
\begin{theorem}
  ...
  \begin{equation*}
    (K \star K')_{n} = \coprod_{[i] \oplus [j] \cong [n]} K_{i} \times K'_{j},
  \end{equation*}
\end{theorem}
\begin{proof}
...
\begin{equation*}
  \sigma = (\alpha \in K_{i}, \beta \in K_{j}, [i] \oplus [j] \cong [n]) \in (K \star K')_{n}
\end{equation*}
to a simplex
\begin{equation*}
  \phi^{*}\sigma = (\psi^{*}_{1} \alpha, \psi^{*}_{2} \beta, \phi^{-1}([i]) \oplus \phi^{-1}([j]) \cong [m]) \in (K \star K')_{m},
\end{equation*}
where
\begin{equation*}
...
\begin{proof}
  ...
  \end{equation*}
  for each $[i] \oplus [j] \cong [n]$. We have a map $f_{i}\colon K_{i} \to K'_{i}$, which gives a map $f_{i} \times \id$ as required. We need only check that this is a morphism of simplicial sets, i.e.\ makes the following square commute for each $\phi\colon [n] \to [m]$.
  \begin{equation*}
    \begin{tikzcd}
      (K \star S)_{m}
      \arrow[r]
    ...
    \end{tikzcd}
  \end{equation*}
  Let $\sigma = (\alpha, \beta, [i] \oplus [j] \cong [n]) \in (K \star S)_{m}$. We follow it around.
  \begin{equation*}
    \begin{tikzcd}
      (\alpha, \beta)
      \arrow[r, mapsto]
...
\begin{theorem}
  ...
  \begin{equation*}
    \Hom_{p}(K \star X, \mathcal{C}) \cong \Hom(K, \mathcal{C}_{/p}),
  \end{equation*}
...
\begin{example}
  ...
  The solution to this lifting problem would give us a homotopy from $\alpha$ to $x$. However, by assumption, there is no such homotopy. Thus, $x$ is not initial in $\Sing(X)$. However, every point $x \in X$ is initial in $\h \Sing(X)$ because $\h \Sing(X) \cong \pi_{\geq 1}(X)$, and there is exactly one homotopy class of paths between any two points of $X$ because $X$ is simply connected.
\end{example}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item First we show the result on simplices. The $n$-simplices of $(\mathcal{C}\op)_{p\op/}$ are
      \begin{align*}
        \{ f\colon K\op \star \Delta^{n} \to \mathcal{C}\op \mid f|_{K\op} = p\op \} &\cong \{ f\op\colon (\Delta^{n})\op \star K \to \mathcal{C} | f\op|_{K} = p \}
      \end{align*}
