...
Saturatedness is a very powerful property. By showing that a certain set of morphisms is saturated, which is often possible based on general principles, the closure properties allow you to manipulate the morphisms in very powerful ways.
...
\begin{definition}[lifting property]
  ...
  \begin{itemize}
    \item The morphism $i$ has the \defn{left lifting property} with respect to the morphism $p$.

    \item The morphism $p$ hs the \defn{right lifting property} with respect to the morphism $i$.
  \end{itemize}
...
In many situations, objects which obey some sort of finiteness condition are especially well-behaved. In the situation of lifting problems, the small object argument, due to Quillen, guarantees us that any set of morphisms whose domains are small provides a \emph{weak factorization system.}
...
\begin{proof}
  ...
  \end{equation*}
  \begin{enumerate}
    \item Consider a morphism in $\mathcal{M}_{2}$ with $e = 0$, i.e.\ an inclusion
      \begin{equation*}
        C^{0} = \{0\} \times \Delta^{n} \coprod_{\{0\} \times \partial \Delta^{n}} \Delta^{1} \times \Delta^{n} \hookrightarrow \Delta^{1} \times \Delta^{n} = C.
      \end{equation*}
      We can draw $C = N([1] \times [n])$ as follows.
      \begin{equation*}
        \begin{tikzcd}
          10
          \arrow[r]
      ...
      An $(n+1)$-simplex in $C$ is a map of posets $[n+1] \to [1] \times [n]$. A $(n+1)$-simplex is nondegenerate if and only if it is a monomorphism. There are exactly $n+1$ of these, of the form
      \begin{equation*}
        ...
        \end{tikzcd}
      \end{equation*}
      We denote the above $(n+1)$-simplex by $\sigma_{i}$.
      ...
      \begin{equation*}
        ...
        \end{tikzcd}
      \end{equation*}
      We see that each of the faces $d_{0} \sigma_{n}$, $d_{1} \sigma_{n}$, etc., is in $C^{0}$, except $d_{n}\sigma_{n}$.
      \begin{equation*}
        \begin{tikzcd}[row sep=small, column sep=small]
          &&&& 1n
          \\
        ...
        \end{tikzcd}
      \end{equation*}
      This says precisely that $\sigma_{n} \cap C^{0} \cong \Lambda^{n+1}_{n}$, giving us a pushout
      \begin{equation*}
        ...
        \end{tikzcd}
      \end{equation*}
      exhibiting $C^{0} \hookrightarrow C^{1} = C^{0} \cup \sigma_{n}$ as a pushout of a horn inclusion. Hence, this inclusion is in $\overline{\mathcal{M}}_{1}$.
      ...
      Proceeding, we see that we have a string of inclusions
      \begin{equation*}
        C^{0} \hookrightarrow C^{1} \hookrightarrow \cdots \hookrightarrow C^{n} = C,
      \end{equation*}
      ...
      \begin{equation*}
        ...
        \end{tikzcd}.
      \end{equation*}
      Thus, the inclusion $C^{0} \hookrightarrow C$ is a composition of morphisms in $\overline{\mathcal{M}}_{1}$, hence is in $\overline{\mathcal{M}}_{1}$
  ...
  It is easy to see that $\overline{M}_{2} \subset \overline{M}_{3}$ since the inclusion $\partial \Delta^{n} \hookrightarrow \Delta^{n}$ is monic, so $\mathcal{M}_{2} \subset \mathcal{M}_{3}$. Conversely,
\end{proof}
